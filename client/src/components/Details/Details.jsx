...
const Details = ({ people, contactToEdit, deleteContact, sortPeople, sortType, sameFirstLastNameCheck }) => {
	...
	const saveEdit = (e) => {
		...
		if (newContact.name.first.length < 1 || newContact.name.last.length < 1) {
			console.log("Contacts must have a first AND last name!");
			setEmptyName(true);
			return;
		} //if there is another contact with the same first name/ last name
		if (sameFirstLastNameCheck(newContact) === true) {
			console.log("prewindow");
			let sameUserConfirmation = window.confirm("A contact with this name already exists. Are you sure you want to continue?");
	...
	const deletePermission = () => {
		let userConfirmation = window.confirm("Are you sure you want to delete this Contact?");
		if (userConfirmation) {
						...
						<Fragment>
							<Form onSubmit={saveEdit}>
								<Form.Group as={Row} className="mb-3" controlId="formPlaintextEmail">
									...
									<Col sm="10">
										<Form.Control defaultValue={contactToEdit.name.first} type="text" />
									</Col>
								</Form.Group>
								<Form.Group as={Row} className="mb-3" controlId="formPlaintextEmail">
									<Form.Label column sm="2">
									...
									<Col sm="10">
										<Form.Control defaultValue={contactToEdit.name.last} type="text" />
									</Col>
								</Form.Group>
								<Form.Group as={Row} className="mb-3" controlId="formPlaintextEmail">
									<Form.Label column sm="2">
									...
									<Col sm="10">
										<Form.Control defaultValue={contactToEdit.email} type="email" />
									</Col>
								</Form.Group>
								<Form.Group as={Row} className="mb-3" controlId="formPlaintextEmail">
									<Form.Label column sm="2">
									...
									<Col sm="10">
										<Form.Control defaultValue={contactToEdit.phone} type="input" />
									</Col>
								</Form.Group>
								<div style={{ padding: "10px", textAlign: "center" }}>
									<Button onClick={deletePermission} variant="primary" type="button">
										Delete Contact
									</Button>
									<div
										style={{
							...
							{emptyName ? <h5 style={{ color: "red", padding: "20px", textAlign: "center" }}>**Contacts must contain a first AND last name</h5> : null}
						</Fragment>
					</Container>
				</>
			)}
